...
# Various variables you might want for your PS1 prompt instead
  ...
  export PS1='$(git branch &>/dev/null;\
    if [ $? -eq 0 ]; then
        ...
        Added=$(trim $(echo "$GitStatus" | grep "^A\{1\}\|^M\{1\}\|^R\{1\}" | wc ));
        Modified=$(trim $(echo "$GitStatus" | grep "^\\sM\|^AM\|^RM\|^MM" | wc ));
        Deleted=$(trim $(echo "$GitStatus" | grep "^\\sD\|^AD" | wc ));
        Untracked=$(trim $(echo "$GitStatus" | grep "^??" | wc ));
        FileChanges="";
        if [ $Added != 0 ]; then
        ...
        echo "\u@ '$BYellow$PathShort$Color_Off' '$IBlue''$Color_Off''$IBlue'"$(__git_ps1 "(%s$FileChanges'$IBlue')") '$Color_Off'$; \
    fi) "
    else \
